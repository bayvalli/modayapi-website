      ...
      <div className="max-w-[1440px] mx-auto px-margin">
        ...
        <div className="grid grid-cols-12 gap-gutter mb-20 items-end">
            ...
            <h1 className="text-[10vw] lg:text-headline-xl text-primary leading-[0.9] uppercase mb-8">
              {project.title.split(' ').map((word, i) => (
                <React.Fragment key={i}>
                  {word}<br className={i === 0 ? "hidden lg:block" : ""} />
                </React.Fragment>
              ))}
            </h1>
          </div>
          <div className="col-span-12 lg:col-span-4 border-l-4 border-primary pl-8 pb-4">
             <p className="text-body-lg text-secondary leading-relaxed">
               {project.description || "Bu proje MAY MODA YAPI'nın yüksek mühendislik standartları ve yenilikçi tasarım anlayışıyla hayata geçirilmiştir."}
             </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-gutter mb-block-gap">
          ...
          <div className="col-span-12 lg:col-span-3">
            <div className="bg-primary text-on-primary p-8 md:p-10 sticky top-32">
              <h3 className="font-serif text-headline-sm uppercase mb-10 border-b border-on-primary/20 pb-4">Teknik Detaylar</h3>
              
              <div className="space-y-8">
                <div className="flex items-start gap-4">
                  <MapPin className="mt-1 opacity-60" size={20} />
                  <div>
                    <span className="font-label-caps text-[10px] tracking-widest opacity-60 block mb-1">LOKASYON</span>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <span className="font-label-caps text-[10px] tracking-widest opacity-60 block mb-1">TOPLAM ALAN</span>
                ...
                {project.units && (
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <span className="font-label-caps text-[10px] tracking-widest opacity-60 block mb-1">BİRİM SAYISI</span>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <span className="font-label-caps text-[10px] tracking-widest opacity-60 block mb-1">ÖZELLİKLER</span>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {project.features.split(',').map((feature, i) => (
                        <span key={i} className="bg-on-primary/10 text-[10px] font-mono px-2 py-1 uppercase">{feature.trim()}</span>
                      ))}
                    </div>
                  </div>
                </div>
        ...
        <div className="max-w-4xl mb-block-gap mt-16">
          <div className="flex items-center gap-6 mb-8 text-primary">
            ...
            <div className="flex-shrink-0 font-label-caps text-sm tracking-widest uppercase">Mimari Yaklaşım</div>
          </div>
        ...
        <div className="border-t-4 border-primary pt-20 mt-20">
          <Link to={`/projeler/${nextProject.id}`} className="group block">
            <span className="font-label-caps text-secondary text-sm tracking-widest mb-4 block uppercase">Sonraki Proje</span>
...
const FloorPlansContainer: React.FC<{ floorPlans: FloorPlan[] }> = ({ floorPlans }) => {
  ...
  const daire7Rooms = [
    { name: 'Lavabo + WC', area: '2.70 m²', x: 10, y: 10, w: 130, h: 60, color: 'rgba(50, 50, 50, 0.05)', type: 'toilet' },
    { name: 'Duş (Ebeveyn)', area: '6.68 m²', x: 10, y: 70, w: 130, h: 60, color: 'rgba(50, 50, 50, 0.05)', type: 'shower' },
    { name: 'Yatak Odası', area: '21.11 m²', x: 10, y: 130, w: 130, h: 140, color: 'rgba(20, 40, 90, 0.05)', type: 'bedroom' },
    { name: 'Balkon 2 (Mutfak)', area: '6.47 m²', x: 10, y: 310, w: 130, h: 80, color: 'rgba(100, 100, 20, 0.04)', type: 'balcony' },
    { name: 'Banyo', area: '6.68 m²', x: 140, y: 10, w: 140, h: 100, color: 'rgba(50, 50, 50, 0.05)', type: 'bath' },
    { name: 'Antre', area: '17.00 m²', x: 140, y: 110, w: 140, h: 100, color: 'rgba(50, 50, 50, 0.02)', type: 'corridor' },
    { name: 'Çocuk Odası', area: '13.81 m²', x: 140, y: 210, w: 140, h: 100, color: 'rgba(10, 80, 110, 0.05)', type: 'bedroom_kid' },
    { name: 'Mutfak', area: '15.37 m²', x: 140, y: 310, w: 140, h: 80, color: 'rgba(80, 20, 80, 0.05)', type: 'kitchen' },
    { name: 'Oturma Odası', area: '17.80 m²', x: 280, y: 10, w: 180, h: 140, color: 'rgba(90, 60, 20, 0.05)', type: 'living' },
    { name: 'Salon', area: '33.74 m²', x: 280, y: 150, w: 180, h: 240, color: 'rgba(10, 80, 50, 0.05)', type: 'salon' },
    { name: 'Balkon 1 (Salon/Oda)', area: '7.30 m²', x: 460, y: 10, w: 30, h: 140, color: 'rgba(100, 100, 20, 0.04)', type: 'balcony' }
  ];

  const daire21DownRooms = [
    // Left column top: Salon (34.11 m2) - size of a large main lounge room
    { name: 'Salon', area: '34.11 m²', x: 25, y: 20, w: 215, h: 140, color: 'rgba(10, 80, 50, 0.05)', type: 'salon' },
    
    // Left column middle: Mutfak (14.80 m2) and Balkon 1 (5.30 m2) on the far-left
    { name: 'Mutfak', area: '14.80 m²', x: 95, y: 170, w: 145, h: 75, color: 'rgba(80, 20, 80, 0.05)', type: 'kitchen' },
    { name: 'Balkon 1', area: '5.30 m²', x: 25, y: 170, w: 60, h: 75, color: 'rgba(100, 100, 20, 0.04)', type: 'balcony' },
    
    // Left column bottom: Oturma Odası (14.12 m2)
    { name: 'Oturma Odası', area: '14.12 m²', x: 25, y: 255, w: 160, h: 115, color: 'rgba(90, 60, 20, 0.05)', type: 'living' },
    
    // Center column bottom: Antre (11.15 m2) which contains the stairs
    { name: 'Antre', area: '11.15 m²', x: 195, y: 255, w: 110, h: 115, color: 'rgba(50, 50, 50, 0.02)', type: 'stairs' },
    
    // Right column top: Yatak Odası (Ebeveyn) (18.19 m2) and its private Duş (3.55 m2)
    { name: 'Yatak Odası (Ebeveyn)', area: '18.19 m²', x: 250, y: 20, w: 215, h: 110, color: 'rgba(20, 40, 90, 0.05)', type: 'bedroom' },
    { name: 'Duş', area: '3.55 m²', x: 250, y: 140, w: 120, h: 50, color: 'rgba(50, 50, 50, 0.05)', type: 'shower' },
    
    // Right column middle: Banyo (5.75 m2) and Balkon 2 (5.75 m2) on the right
    { name: 'Banyo', area: '5.75 m²', x: 250, y: 200, w: 80, h: 45, color: 'rgba(50, 50, 50, 0.05)', type: 'bath' },
    { name: 'Balkon 2', area: '5.75 m²', x: 340, y: 200, w: 125, h: 45, color: 'rgba(100, 100, 20, 0.04)', type: 'balcony' },
    
    // Right column bottom: Çocuk Odası (9.15 m2) and Lavabo + WC (1.55 m2)
    { name: 'Çocuk Odası', area: '9.15 m²', x: 360, y: 255, w: 105, h: 115, color: 'rgba(10, 80, 110, 0.05)', type: 'bedroom_kid' },
    { name: 'Lavabo + WC (LVR-WC)', area: '1.55 m²', x: 310, y: 255, w: 45, h: 65, color: 'rgba(50, 50, 50, 0.05)', type: 'toilet' }
  ];

  const daire21UpRooms = [
    // Top Left: Üst Kat Salon (45.00 m2) (Includes open dining and lounge furniture renderer)
    { name: 'Üst Kat Salon', area: '45.00 m²', x: 95, y: 20, w: 375, h: 170, color: 'rgba(10, 80, 50, 0.05)', type: 'salon' },
    
    // Bottom Left: Teras (20.31 m2)
    { name: 'Teras', area: '20.31 m²', x: 25, y: 200, w: 165, h: 140, color: 'rgba(100, 100, 20, 0.04)', type: 'balcony' },
    
    // Center bottom: Merdiven (Duplex Stairs) leading to Antre downstairs
    { name: 'Stairs', area: '21', x: 200, y: 200, w: 75, h: 140, color: 'rgba(50, 50, 50, 0.02)', type: 'stairs' },
    
    // Hol / Koridor (8.32 m2)
    { name: 'Hol / Koridor', area: '8.32 m²', x: 285, y: 200, w: 70, h: 60, color: 'rgba(50, 50, 50, 0.02)', type: 'corridor' },
    
    // Duş (3.33 m2) next to Hol
    { name: 'Duş', area: '3.33 m²', x: 285, y: 265, w: 70, h: 75, color: 'rgba(50, 50, 50, 0.05)', type: 'shower' },
    
    // Bottom Right: Oda (15.75 m2)
    { name: 'Oda', area: '15.75 m²', x: 365, y: 200, w: 105, h: 140, color: 'rgba(20, 40, 90, 0.05)', type: 'bedroom' }
  ];

  const currentRooms = activePlan.layoutType === 'flat' 
    ? activePlan.rooms 
    : (activeFloor === 'down' ? activePlan.downstairsRooms : activePlan.upstairsRooms);

  const currentSVGLayout = activePlan.layoutType === 'flat'
    ? daire7Rooms
    : (activeFloor === 'down' ? daire21DownRooms : daire21UpRooms);
  ...
  const renderRoomInterior = (room: any) => {
    ...
    switch(type) {
      case 'bedroom': {
        ...
        return (
          <g opacity="0.15" stroke="currentColor" strokeWidth="1" fill="none" className="pointer-events-none">
            {/* Bed outline */}
            <rect x={bedX} y={bedY} width={bedW} height={bedH} rx="2" />
            <line x1={bedX} y1={bedY + bedH * 0.4} x2={bedX + bedW} y2={bedY + bedH * 0.4} />
            <path d={`M ${bedX} ${bedY + bedH * 0.4} Q ${bedX + bedW / 2} ${bedY + bedH * 0.5} ${bedX + bedW} ${bedY + bedH * 0.4}`} />
            {/* Pillows */}
            <rect x={bedX + 8} y={bedY + 8} width={bedW * 0.35} height={bedH * 0.2} rx="1" />
            <rect x={bedX + bedW - 8 - bedW * 0.35} y={bedY + 8} width={bedW * 0.35} height={bedH * 0.2} rx="1" />
          </g>
        );
      }
      case 'bedroom_kid': {
      ...
      case 'salon': {
        ...
        return (
          <g opacity="0.15" stroke="currentColor" strokeWidth="1" fill="none" className="pointer-events-none">
            <path d={`M ${sofX} ${sofY} L ${sofX + sofW} ${sofY} L ${sofX + sofW} ${sofY + 25} L ${sofX + 25} ${sofY + 25} L ${sofX + 25} ${sofY + sofH} L ${sofX} ${sofY + sofH} Z`} />
      ...
      case 'balcony': {
        ...
        return (
          <g opacity="0.12" stroke="currentColor" strokeWidth="0.75" className="pointer-events-none">
            {lines}
          </g>
        );
      }
      case 'stairs': {
        ...
        for (let i = 1; i < stepsCount; i++) {
          stepLines.push(
            <line key={i} x1={x} y1={y + i * stepHeight} x2={x + w} y2={y + i * stepHeight} stroke="currentColor" strokeWidth="1" opacity="0.25" />
          );
        }
        return (
          <g className="pointer-events-none">
            ...
            <path d={`M ${x + w/2} ${y + h - 10} L ${x + w/2} ${y + 15}`} stroke="currentColor" strokeWidth="1.2" fill="none" strokeDasharray="2 2" opacity="0.4" />
            {/* Çember içerisindeki 21 No'lu Mimari No Logosu */}
            <circle cx={x + w/2} cy={y + h/2} r="15" fill="var(--color-primary)" stroke="currentColor" strokeWidth="1" />
            <text x={x + w/2} y={y + h/2 + 4} textAnchor="middle" fontSize="10" className="fill-current text-white font-sans font-black select-none">21</text>
          </g>
        );
      }
      default:
  ...
  return (
    <div className="mt-20 border-t-4 border-primary pt-16">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <span className="font-label-caps text-secondary text-sm tracking-[0.25em] mb-2 block uppercase">Örnek Kat Planları</span>
          <h2 className="font-serif text-headline-md text-primary uppercase leading-tight">Mekansal Dağılım & Ölçüler</h2>
        </div>
        ...
        <div className="col-span-12 lg:col-span-5 flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b-2 border-primary pb-3 mb-6 bg-primary/5 p-4">
              <div>
                <span className="font-serif text-headline-sm text-primary uppercase block">{activePlan.name}</span>
                <span className="font-sans text-xs text-secondary/70 uppercase block tracking-wider">{activePlan.type}</span>
              </div>
              <div className="text-right">
                <span className="font-mono text-xs uppercase block opacity-60">Toplam Brüt Alan</span>
                <span className="font-sans font-black text-2xl text-primary">{activePlan.totalArea}</span>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full ${isHovered ? 'bg-on-primary' : 'bg-primary'}`}></div>
                      <span className="font-sans font-semibold text-body-md uppercase tracking-wide">{room.name}</span>
                    </div>
                    <span className={`font-mono text-sm ${isHovered ? 'text-on-primary/95 font-bold' : 'text-secondary/75'}`}>{room.area}</span>
                  </div>
                );
              })}
            </div>
                ...
                {viewerMode === 'image' && activePlan.imageUrl ? (
                  ...
                  >
                    ...
                    <g transform="translate(460, 40)" opacity="0.3" className="pointer-events-none">
                      ...
                      <text x="0" y="-30" textAnchor="middle" fontSize="10" className="font-mono fill-current font-bold">N</text>
                    </g>

                    {/* Legend/Technical Info */}
                    <g transform="translate(20, 380)" opacity="0.4" className="pointer-events-none font-mono" fontSize="8" letterSpacing="0.1em">
                      ...
                      return (
                        ...
                        >
                          ...
                          <rect
                            ...
                            width={room.w}
                            height={room.h}
                            fill={isHovered ? 'rgba(0, 0, 0, 0.08)' : room.color || 'rgba(0,0,0,0.02)'}
                            stroke="currentColor"
