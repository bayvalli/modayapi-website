...
const AccordionItem: React.FC<{ q: string; a: string; isOpen: boolean; onClick: () => void }> = ({ q, a, isOpen, onClick }) => {
  return (
    <div className="border-b-4 border-primary overflow-hidden">
      ...
      >
        <h3 className={`font-serif text-headline-sm text-primary uppercase transition-all duration-300 ${isOpen ? 'pl-4 text-secondary' : 'group-hover:pl-4'}`}>
          {q}
        </h3>
    ...
    >
      <div className="max-w-4xl mx-auto">
        <header className="mb-20 text-center">
          <span className="font-label-caps text-secondary text-sm tracking-[0.3em] mb-4 block uppercase">Bilgi Merkezi</span>
          <h1 className="font-serif text-headline-xl text-primary uppercase leading-tight">Sıkça Sorulan<br/>Sorular</h1>
        ...
        <div className="mt-24 p-12 bg-primary/5 text-center">
          <p className="font-serif text-headline-sm text-primary uppercase mb-6">Aradığınız cevabı bulamadınız mı?</p>
          <Link 
            to="/iletisim" 
