      ...
      <section className="mt-block-gap px-margin relative">
        <div className="max-w-[1440px] mx-auto">
          <header className="mb-24 text-center">
            <h2 className="font-serif text-headline-xl text-primary uppercase">FAALİYET<br />KONULARI</h2>
          </header>
            ...
            {[
              ...
              <div key={i} className={`relative group ${i % 2 === 1 ? 'md:mt-32' : ''}`}>
                <div className="w-full h-[400px] border-4 border-primary relative overflow-hidden bg-secondary z-10">
                  <img src={item.img} alt={item.title} className="w-full h-full object-cover grayscale mix-blend-multiply opacity-80" />
                </div>
                <div className={`bg-surface border-4 border-primary p-8 absolute -bottom-16 ${i % 2 === 0 ? '-right-8' : '-left-8'} w-4/5 z-20 shadow-xl`}>
                  <h3 className="font-serif text-headline-md font-bold text-primary mb-2 tracking-tight">{item.title}</h3>
        ...
        <div className="max-w-[1440px] mx-auto grid grid-cols-12 gap-gutter relative">
          ...
          <div className="col-span-12 md:col-span-6 md:col-start-6 z-20 -mt-32 md:-mt-64 relative">
             <div className="bg-surface border-4 border-primary p-12">
                <h2 className="font-serif text-headline-lg text-primary mb-8 leading-tight">Mühendislik<br />Gücü.</h2>
                <p className="font-sans text-body-lg text-on-surface mb-8">
                  Kaliteden ödün vermeden, yenilikçi ve sağlam mühendislik çözümleri üretiriz. 
                  Tüm süreçlerimizde açık iletişim kurar, paydaşlarımızla güvene dayalı ilişkiler inşa ederiz.
                </p>
