        ...
        <div className="flex flex-col gap-block-gap">
          {PROJECTS.map((project, index) => (
            <motion.div 
              ...
              className={`grid grid-cols-12 gap-gutter relative ${index % 2 === 1 ? 'md:flex-row-reverse' : ''}`}
            >
              {/* Image Column */}
              <div className={`col-span-12 md:col-span-8 ${index % 2 === 1 ? 'md:col-start-5' : ''}`}>
              ...
              {/* Info Block - Absolute/Relative mix for Brutalist overlap */}
              <div className={`col-span-12 md:col-span-5 bg-surface border-heavy p-8 md:p-12 z-20 shadow-none mt-8 md:mt-0 ${index % 2 === 0 ? 'md:absolute md:bottom-[-40px] md:right-0' : 'md:absolute md:bottom-[-40px] md:left-0'}`}>
                <span className="font-label-caps text-secondary mb-4 block uppercase">{project.category}</span>
                <h3 className="text-headline-lg text-primary mb-6 leading-tight">{project.title}</h3>
                ...
                <Link to={`/projeler/${project.id}`}>
                  <BrutalistButton variant="primary" className="flex items-center gap-4 w-full md:w-auto justify-center">
