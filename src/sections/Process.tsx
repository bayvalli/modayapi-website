...
export const Process: React.FC = () => {
  return (
    <section
      id="process"
      className="py-block-gap bg-surface-container-low border-y-4 border-primary"
    >
      <div className="max-w-[1440px] mx-auto px-margin">
        <div className="max-w-2xl border-l-4 border-primary pl-8 mb-block-gap">
          <h2 className="text-headline-xl text-primary lowercase mb-6">
            Anıtsal
            <br />
            Adımlar.
          </h2>
          ...
          {PROCESS_STEPS.map((step, index) => (
            ...
            >
              ...
              <div className="md:w-2/3 p-margin flex flex-col justify-center">
                <h3 className="text-headline-lg mb-6 leading-none group-hover:text-on-primary transition-colors">
                  {step.title}
                </h3>
                ...
                <ul className="flex flex-wrap gap-4">
                  {step.items.map((item) => (
                    <li
                      key={item}
                      className="font-label-caps text-label-caps border-2 border-primary group-hover:border-on-primary px-4 py-2 uppercase group-hover:text-on-primary transition-colors"
                    >
                      {item}
                    </li>
                  ))}
