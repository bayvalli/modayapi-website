      ...
      <div className="max-w-[1440px] mx-auto px-margin">
        <header className="mb-block-gap flex flex-col md:flex-row justify-between items-end gap-gutter">
          ...
          <div className={`md:w-1/3 ${isModern ? 'border-b-4' : 'border-b-4'} border-primary pb-4`}>
            <p className={`text-body-md text-secondary lowercase ${isModern ? 'text-sm' : ''}`}>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      <span className={`text-body-md font-bold ${isModern ? 'text-sm' : ''}`}>
                        {project.location}
                      </span>
                    </div>
                    <div>
                    ...
                    <div className={isModern ? 'col-span-1' : 'col-span-2'}>
                      <span
                        className={`font-label-caps text-secondary block mb-1 ${isModern ? 'text-[9px]' : ''}`}
                      >
                        {isModern ? 'ÖZ.' : 'ÖZELLİKLER'}
                      </span>
                      <span className={`text-body-md font-bold ${isModern ? 'text-[11px]' : ''}`}>
                        {isModern ? project.features.substring(0, 20) + '...' : project.features}
                      </span>
                    </div>
                  </div>
                  ...
                  <Link to={`/projeler/${project.id}`}>
                    ...
                    >
                      {isModern ? 'İNCELE' : 'PROJEYİ İNCELE'} <ArrowRight size={18} />
                    </BrutalistButton>
                  </Link>
                </div>
              </motion.div>
