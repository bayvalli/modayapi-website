...
export const ZoomLightbox: React.FC<ZoomLightboxProps> = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        onClose();
      } else if (e.key === 'ArrowLeft' && onNavigate && activeIndex !== undefined && activeIndex > 0) {
        onNavigate(activeIndex - 1);
      } else if (e.key === 'ArrowRight' && onNavigate && activeIndex !== undefined && images && activeIndex < images.length - 1) {
        onNavigate(activeIndex + 1);
      }
    };
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="text-center py-2 border-t border-on-primary/10 mt-2 flex flex-col sm:flex-row items-center justify-between font-mono text-[9px] text-on-primary/40 uppercase tracking-widest gap-2 bg-on-primary/[0.02] px-4">
          ...
          <span>{scale > 1 ? '🖱️ Sürükleyerek taranabilir // 📱 Çift dokunun' : '🖱️ Çift tıklayarak veya tekerlekle yakınlaştırın'}</span>
